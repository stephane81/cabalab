...
type courbe = record
   ...
   ADD          : boolean;
   mode         : ( ch1 , ch2 , chop );
   Tmode         : array [0..2] of boolean;     //[0]=true : les 4   ; [i]=true : la voie i +...
   XY          : boolean;
   couleur1     : Tcolor;
   couleur2     : Tcolor;
   nom          : string[255];
...
type
  ...
  TOscilloCaba = class(TForm)
    ...
    Bassersolution1: TMenuItem;
    Hautersolution1: TMenuItem;
    Regressi_Inst_BR_NF: TMenuItem;
    Regressi_Inst_BR_NP: TMenuItem;
    Regressi_Inst_HR_NF: TMenuItem;
    Regressi_Inst_HR_NP: TMenuItem;
    N1: TMenuItem;
    ...
    procedure EnvoieDonneesHS3( ch1,ch2,HauteResol,NewFichier:boolean );
    procedure TRANSMulti(Sender: TObject);
    procedure EnvoieDonneesHS3_Enreg_Multi(NoEnrg:integer;diviseur:integer;ALL,Ch1,Ch2:boolean);
    procedure RegressiMONOClick(Sender: TObject);
    procedure EnvoieDonneesHS3_Enreg_Mono(diviseur:integer);
...
{$IFDEF CABALABin}
uses UnitOscilloHS3optionsEnrg,UnitOscilloHS3NbEchPerso,UnitOscilloHS3TransfertRegressi,UnitTransfertRegressi, UnitCabalab, UnitApropos,clipbrd,
  UnitGBF;
{$ENDIF}

{$IFDEF Independant}
uses UnitOscilloHS3optionsEnrg,UnitOscilloHS3NbEchPerso,UnitOscilloHS3TransfertRegressi,UnitTransfertRegressi,regressispecdecl,clipbrd;      //méthode intermédiaire copie des blocs...rregressi
{$ENDIF}
...
//   end;
   ...
   //SetTriggerMode (  ltkrising ) ;  // trigger sur front montant
   //SetTriggerLevel ( lCh1 , 0 ) ;
   SetTriggerSource ( 9 ) ; //trigger defaut no trig
...
begin
    FormUTILISATEUR_RegressiTransfert.visible:=true;
end;

procedure TOscilloCaba.RegressiDDEClick(Sender: TObject);
begin
    //formDDE.visible:=true;
    //formDDE.BringToFront;

     if sender=Regressi_Inst_BR_NF then
     begin
          EnvoieDonneesHS3( CH1actif,CH2actif,false,true );
     end;
     if sender=Regressi_Inst_HR_NF then
     begin
          EnvoieDonneesHS3( CH1actif,CH2actif,true,true );
     end;
     if sender=Regressi_Inst_BR_NP then
     begin
          EnvoieDonneesHS3( CH1actif,CH2actif,false,false );
     end;
     if sender=Regressi_Inst_HR_NP then
     begin
          EnvoieDonneesHS3( CH1actif,CH2actif,true,false );
     end;

end;

procedure TOscilloCaba.EnvoieDonneesHS3( ch1,ch2,HauteResol,NewFichier:boolean );
var i : integer;
    ligne : string;
    BouLigne : string;
    zero,coeff : single;
...
begin
     ...
     begin
        ...
        //  if GTI2 then Add('Appareil utilisé : Orphy GTI2');
          ...
          ligne := 't';
         { if ch1 then
                      if congiguration.Unipolaire[1]=true then ligne :=ligne +crTab+'CH1' else ligne :=ligne +crTab+'YD0';
          if ch2 then
                      if congiguration.Unipolaire[2]=true then ligne :=ligne +crTab+'CH2' else ligne :=ligne +crTab+'YD1';
          }
...

       {
          ...
          if HauteResol=false then
             begin
             for i:=0 to round(Recordlength/10)-1 do
                 begin
                 ligne :=FloatToStr(10*i/freq);
                 if ch1 then ligne :=ligne +crTab+FloatToStr(Ch1DoubleArray [i*10]);
                 if ch2 then ligne :=ligne +crTab+FloatToStr(Ch2DoubleArray [i*10]);
                 Add(ligne);
                 end;
             end;
...
procedure TOscilloCaba.TRANSMulti(Sender: TObject);
...
begin
     ...
     if nb_courbes_stockees<>0 then
        ...
        for j:=1 to nb_courbes_stockees do
            begin
            sleep(250);
            EnvoieDonneesHS3_Enreg_Multi({NoEnrg:integer}j,
...
procedure TOscilloCaba.EnvoieDonneesHS3_Enreg_Multi(NoEnrg:integer;diviseur:integer;ALL,Ch1,Ch2:boolean);
var i,j,k : integer;
    ligne : string;
    BouLigne : string;
...
k:=0;
     ...
     begin
      ...
      else   //if ALL    marche pas pour l'instant
      begin
          ligne := 't';
          if ch1 then ligne :=ligne +crTab+ 'CH1';
          if ch2 then ligne :=ligne +crTab+ 'CH2';
...
procedure TOscilloCaba.RegressiMONOClick(Sender: TObject);
begin
     EnvoieDonneesHS3_Enreg_Mono(10);
end;

procedure TOscilloCaba.EnvoieDonneesHS3_Enreg_Mono(diviseur:integer);
var i,j : integer;
    ligne : string;
    ligne2: string;
    BouLigne : string;
...
//NOTrigButton.Checked:=true;
//NOTrigButtonClick(sender);
//NOTrigButtonClick(NOTrigButton);
